...
async function getScopeFilter(tenantId, userId, role, scopeOrgId, scopeRole, tableAlias = 'ch') {
  // Nivel 1: Admin Global (No aplica filtros extra)
  if (role === 'admin' && !scopeOrgId) {
...
app.post('/api/backoffice/auth', async (req, res) => {
  ...
  try {
    // 1. Intentar primero en nuestra base de datos local (para usuarios admin de Bantos)
    const [rows] = await pool.query(
    ...
    if (rows.length > 0) {
      ...
      if (user.password) {
        ...
        if (valid) {
          ...
          try {
            await pool.query('INSERT INTO operation_logs (user_id, tenant_id, process_type, process_id, status) VALUES (?, ?, ?, ?, ?)', [user.id, user.tenant_id, 'USER_LOGIN', user.username, 'SUCCESS']);
    ...
    // 2. Validación Dinámica contra Upya
    // Si Upya acepta las credenciales, verificamos el acceso al tenantId
    const baseUrl = process.env.UPYA_BASE_URL || 'https://api.upya.io';
    try {
      // Intentamos una operación en Upya para validar credenciales Y acceso al tenant
      // Nota: En Upya, el tenantId suele usarse como un header o parámetro de contexto en implementaciones personalizadas,
      // pero aquí lo validamos asegurándonos de que la sesión sea válida y el tenant sea c-romel o bantoshub (según tu registro)
      ...
      if (upyaRes.status === 200) {
        // VALIDACIÓN ESTRICTA: Solo permitimos tenants que sabemos que existen para este usuario
        // o que están registrados oficialmente.
        const allowedTenants = ['c-romel', 'bantoshub', 'sotelo']; 
        
        if (!allowedTenants.includes(tenantId)) {
           return res.status(403).json({ success: false, message: 'El tenant no está registrado o no tienes acceso.' });
        }

        // Registro automático o actualización del tenant en nuestra base local
        await pool.query('INSERT IGNORE INTO tenants (tenant_id) VALUES (?)', [tenantId]);
      ...
      if (upyaErr.response?.status === 401) {
        return res.status(401).json({ success: false, message: 'Usuario o contraseña incorrectos en Upya.' });
      }
    }

    return res.status(401).json({ success: false, message: 'El tenant no pudo ser validado con Upya.' });

    return res.status(401).json({ success: false, message: 'El tenant no está registrado o las credenciales son inválidas.' });
...
app.post('/api/backoffice/products', async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      `INSERT INTO products (upya_id, tenant_id, name, category, reference, is_lockable, manufacturer, is_serialized, description, status, picture_url, tac, build, default_managed_by, base_value, vat_rate) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
