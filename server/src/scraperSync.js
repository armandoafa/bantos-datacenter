export async function syncScrapedData(scrapedData, pool, tenantId) {
  ...
  for (const row of (scrapedData.clients || [])) {
    try {
      ...
      const clientNumber = row.find(c => /^C\d{6,12}$/i.test(c)) || row[0];
      const status = row.find(c => ['Signed', 'Approved', 'Pending', 'Rejected', 'Active'].includes(c)) || 'Signed';
      const signingDateStr = row.find(c => c.includes('2025') || c.includes('2026') || c.includes('Jan') || c.includes('Feb') || c.includes('Mar') || c.includes('Apr') || c.includes('May') || c.includes('Jun') || c.includes('Jul') || c.includes('Aug') || c.includes('Sep') || c.includes('Oct') || c.includes('Nov') || c.includes('Dec'));
      const signingDate = parseDate(signingDateStr);

      const firstName = row[1] || '';
      const lastName = row[2] || '';
      const fullName = (firstName + ' ' + lastName).trim() || row.find(c => /^[a-zA-Z\s]{4,}$/.test(c)) || 'Cliente ' + clientNumber;
      
      const entity = row[3] || row.find(c => ['PLAYZONE', 'Credicell', 'TecnoPhone'].includes(c) || (c === c.toUpperCase() && c.length > 3 && c !== status)) || null;
      const agent = row[row.length - 1] && !row[row.length - 1].startsWith('URL:') && !row[row.length - 1].startsWith('SIG:') ? row[row.length - 1] : null;
      ...
      await pool.query(
        `INSERT INTO client_history 
         (upya_id, client_number, tenant_id, name, first_name, last_name, entity, agent, status, signing_date) 
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) 
         ...
         entity=COALESCE(VALUES(entity), entity),
         agent=COALESCE(VALUES(agent), agent),
         status=VALUES(status),
         signing_date=COALESCE(VALUES(signing_date), signing_date)`,
        [upyaId, clientNumber, tenantId, fullName, firstName, lastName, entity, agent, status, signingDate]
      );
    } catch(e) {}
  }
