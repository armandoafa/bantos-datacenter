...
export default function ConfigSetup({ session }) {
  ...
  const fetchSettings = async () => {
    try {
      const res = await axios.get(`${API}/backoffice/settings/smtp?tenantId=${session.tenantId}`);
      if (res.data) {
        setFormData({
  ...
  const handleSave = async () => {
    ...
    setSuccessMsg('');
    try {
      await axios.post(`${API}/backoffice/settings/smtp`, {
    ...
    <div className="animate-in fade-in duration-300 max-w-4xl mx-auto">
      ...
      <div className="bg-white rounded-[24px] border-2 border-slate-100 shadow-sm overflow-hidden mb-6">
        ...
        <div className="p-6 border-t border-slate-100 bg-slate-50/50 flex items-center justify-between">
          <div>
            {successMsg && <span className="text-emerald-600 font-bold text-sm">{successMsg}</span>}
          </div>
          <button 
            onClick={handleSave}
            disabled={loading}
